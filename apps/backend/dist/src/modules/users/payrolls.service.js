...
let PayrollsService = class PayrollsService {
    ...
    async findAll(businessId) {
        return this.db
            .select()
            .from(user_schema_1.payrolls)
            .where((0, drizzle_orm_1.eq)(user_schema_1.payrolls.businessId, businessId));
    }
    async findOne(businessId, id) {
        const [payroll] = await this.db
            .select()
            .from(user_schema_1.payrolls)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(user_schema_1.payrolls.id, id), (0, drizzle_orm_1.eq)(user_schema_1.payrolls.businessId, businessId)));
        if (!payroll) {
    ...
    async update(businessId, id, dto) {
        ...
        const [updated] = await this.db
            .update(user_schema_1.payrolls)
            .set({ ...dto, updatedAt: new Date() })
            .where((0, drizzle_orm_1.eq)(user_schema_1.payrolls.id, id))
            .returning();
        return updated;
    }
    async remove(businessId, id) {
        await this.findOne(businessId, id);
        await this.db.delete(user_schema_1.payrolls).where((0, drizzle_orm_1.eq)(user_schema_1.payrolls.id, id));
        return { success: true };
    }
};
